...
function AllSubModules(val){
	document.getElementById("submoduleName").innerHTML="";
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			var data="<option value="+"0"+" selected>See Assigned submodule<oprion>";
			var jsonData=JSON.parse(this.responseText);
			for(var i=0;i<jsonData.length;i++){
				data+="<option value="+jsonData[i].submoduleid+">"+jsonData[i].submodulename+"<oprion>";
			}
			document.getElementById("submoduleName").innerHTML=data;
		}
	};
...
function GetAllModules(val){
	...
	xhttp.open("POST", "GetAllTModules?projectId="+val.value, true);
	xhttp.send();
}


function AllTask(val)
{
	var xhttp;
	if(val.value == ""){
	...
	xhttp.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
			document.getElementById("assignedtasks").innerHTML = this.responseText;
		}
	};
	  xhttp.open("GET", "alltasks?submoduleId="+val.value, true);           
	  xhttp.send();
}
 ...
 <div class="affix" style="width:100%;" >
	...
	<div class="container-fluid" style="padding:250px 50px 100px 50px;min-height:738px;">
			...
			<table id="main-fourum" class="table table-striped table-bordered">
				...
				<tbody>
				<c:forEach items="${projectlist }" var="project1" varStatus="loop">
				<tr>
				<td>${loop.index+1 }</td>
				<td>${project1.projectName }</td>
				<td>${project1.moduleNameIndividually }</td>
				<td>${project1.subModuleNameIndividually }</td>
				<td>${project1.taskNameIndividually }</td>
				<td>${project1.taskDescriptionIndividually }</td>
