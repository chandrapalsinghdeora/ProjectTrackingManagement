...
@Repository
public class AdminViewProgressDaoImpl implements AdminViewProgressDao {
	...
	public List<Project> getAllProject(int userid) {
		...
		final String procedureCall = "{call proc_adminviewprogress(?,?,?,?)}";
	...
	public JSONArray getsubmodules(int moduleid) {
		...
		try {
			...
			JSONObject obj = null;
			int x=1;
			System.out.println("ppp rs.next:::");
			while(rs.next()){
				obj = new JSONObject();
