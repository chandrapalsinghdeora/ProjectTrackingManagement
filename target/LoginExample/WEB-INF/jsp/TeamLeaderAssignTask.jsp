...
function GetAllTask(val){
	document.getElementById("taskId").innerHTML="";
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			var data="<option value="+"0"+" selected>tasks<oprion>";
			var jsonData=JSON.parse(this.responseText);
			for(var i=0;i<jsonData.length;i++){
				data+="<option value="+jsonData[i].pkId+">"+jsonData[i].name+"<oprion>";
			}
	...
	xhttp.open("POST", "Getalltask?subModuleId="+val.value, true);
	xhttp.send();
}
</script>
...
function GetAllEmployee(val){
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			var data="<option value="+"0"+" selected>employees<oprion>";
			var jsonData=JSON.parse(this.responseText);
			for(var i=0;i<jsonData.length;i++){
				data+="<option value="+jsonData[i].userId+">"+jsonData[i].name+"<oprion>";
			}
			document.getElementById("userId").innerHTML=data;
		}
	};
	xhttp.open("POST", "Getallemployee?taskId="+val.value, true);
	xhttp.send();
}
</script>
   ...
   <div class="container">
    ...
    <div class="panel panel-primary" style="width:600px;border:none;margin-left:306px;">
      ...
      <div class="panel-body" style="padding: 0px 90px;">
         ...
         <form class="form-signin" action="assigntasktoemp" method="post">  
         ...
         Sub Modules<sup>*</sup>
	      <select class="form-control" name="subModuleId" id="submoduleId" onchange="GetAllTask(this);" required >
	      <option value="0" selected>Assigned SubModule</option>	
          </select> <br>
          
          
          Task<sup>*</sup>
	      <select class="form-control" name="taskId" id="taskId" onchange="GetAllEmployee(this);" required >
